...
import TableComponent, { TableComponentProps } from '../TableComponent/TableComponent';
        ...
        if (originalData.length > 0) {
            ...
            Object.keys(sampleRow).forEach(key => {
                ...
                if (uniqueValues.length > 1 && uniqueValues.length < 50) {
                    ...
                    } else if (key.toLowerCase().includes('estatus') || key.toLowerCase().includes('status')) {
                        configs.push(FilterPresets.createStatusFilter(key, uniqueValues));
                    } else if (typeof uniqueValues[0] === 'string') {
                        configs.push(FilterPresets.createSelectFilter(
                            key,
        ...
        if (Object.keys(activeFilters).length > 0) {
            data = data.filter(row => {
                return Object.entries(activeFilters).every(([filterKey, filterValue]) => {
                    ...
                    return String(rowValue).toLowerCase().trim() === String(filterValue).toLowerCase().trim();
                });
            });
        }
...
export const FilterableTableExample: React.FC = () => {
    const sampleData = [
        { id: 1, fecha: '2024-01-15', encargado: 'Juan Pérez', estatus: 'Agendado', precio: 1500 },
        { id: 2, fecha: '2024-02-10', encargado: 'María García', estatus: 'En Curso', precio: 2000 },
        { id: 3, fecha: '2024-03-05', encargado: 'Carlos López', estatus: 'Finalizado', precio: 1800 },
        { id: 4, fecha: '2024-01-20', encargado: 'Juan Pérez', estatus: 'Por Pagar', precio: 2200 },
        { id: 5, fecha: '2024-02-28', encargado: 'María García', estatus: 'Agendado', precio: 1700 }
    ];
    ...
    const customFilters: FilterConfig[] = [
        FilterPresets.createDateFilter('fecha', sampleData.map(d => d.fecha)),
        FilterPresets.createSelectFilter('encargado', 'Encargado', ['Juan Pérez', 'María García', 'Carlos López']),
        FilterPresets.createStatusFilter('estatus', ['Agendado', 'En Curso', 'Finalizado', 'Por Pagar']),
        {
            key: 'precio',
