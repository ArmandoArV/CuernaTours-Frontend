...
const AssignDriverModal: React.FC<AssignDriverModalProps> = ({
  ...
  const [vehicleOptions, setVehicleOptions] = useState<Array<{ value: string; label: string; licensePlate?: string }>>([]);
  ...
  const getCurrentDriverId = (): string => {
    ...
    if ('driver' in data && data.driver) {
      if (typeof data.driver === 'object' && ('id' in data.driver || 'user_id' in data.driver)) {
  ...
  const getCurrentVehicleId = (): string => {
    ...
    if ('vehicle' in data && data.vehicle) {
      if (typeof data.vehicle === 'object' && ('id' in data.vehicle || 'vehicle_id' in data.vehicle)) {
  ...
  const getExternalProviderInfo = (): { providerName: string; driverName: string; contact: string; vehicleType: string } => {
    ...
    const providerKeys = ['Proveedor', 'proveedor', 'provider_name', 'providerName'];
    const driverNameKeys = ['Chofer externo', 'chofer_externo', 'external_driver_name', 'externalDriverName'];
    const contactKeys = ['Contacto', 'contacto', 'contact', 'phone'];
    const vehicleTypeKeys = ['Tipo de unidad', 'tipo_unidad', 'vehicleType', 'unit_type'];
  ...
  const matchDriverByName = (driverName: string, drivers: DriverReference[]): string | null => {
    ...
    return matchedDriver ? (matchedDriver.user_id || matchedDriver.id)?.toString() || null : null;
  };

  // Match vehicle by name and return ID and plate
  const matchVehicleByName = (vehicleName: string, vehicles: VehicleReference[]): { id: string | null; plate: string } => {
    ...
    return matchedVehicle 
      ? { 
          id: (matchedVehicle.vehicle_id || matchedVehicle.id)?.toString() || null, 
  ...
  const getTripType = (): string => {
    ...
    const tripTypeKeys = ['Tipo de viaje', 'Tipo', 'tipo', 'Trip Type', 'tripType', 'trip_type', 'type', 'TipoViaje'];
    ...
    // Check for TripCollection
    if ('tripType' in tripData && typeof (tripData as TripCollection).tripType === 'string') {
  ...
  const getOriginName = (): string => {
    ...
    if ('origin' in data && data.origin && typeof data.origin === 'object' && 'name' in data.origin) {
      return data.origin.name;
    }
    // Check for direct origin properties
    const originKeys = ['Origen', 'origen', 'Origin', 'origin_name', 'originName', 'origin'];
  ...
  const getDestinationName = (): string => {
    ...
    if ('destination' in data && data.destination && typeof data.destination === 'object' && 'name' in data.destination) {
      return data.destination.name;
    }
    // Check for direct destination properties
    const destinationKeys = ['Destino', 'destino', 'Destination', 'destination_name', 'destinationName', 'destination'];
  ...
  const getServiceDate = (): Date => {
    ...
    const dateKeys = ['Fecha', 'fecha', 'Date', 'service_date', 'serviceDate', 'departure_date', 'departureDate', 'date', 'Fecha de Servicio', 'Fecha de salida'];
  ...
  const getPassengers = (): number => {
    ...
    const passengerKeys = ['Pasajeros', 'pasajeros', 'Passengers', 'passengers', 'No. de Pasajeros', 'No. Pasajeros', 'passenger_count', 'passengerCount'];
    ...
    // Check for TripCollection
    if ('totalPassengers' in tripData && typeof (tripData as TripCollection).totalPassengers === 'number') {
  ...
  const getCurrentVehicleInfo = (): string | null => {
    ...
    if ('vehicle' in data && data.vehicle && typeof data.vehicle === 'object') {
      ...
      if ('license_plate' in vehicle && vehicle.license_plate) parts.push(`(${vehicle.license_plate})`);
        ...
        {isLoading ? (
          ...
          {(currentDriverName || currentVehicleInfo || currentPlate) && (
            <div className={styles.currentAssignment}>
              <p><strong>Asignación actual:</strong></p>
              {currentDriverName && (
                <p>
              ...
              {currentPlate && !currentVehicleInfo?.includes(currentPlate) && (
                <p>
